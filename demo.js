(() => {
  ...
  function renderIcons(filter = "") {
    ...
    document.querySelectorAll(".symbol-wrapper").forEach((icon) => {
      icon.addEventListener("click", (e) => {
        e.stopPropagation();
        symbolClick(e.target.getAttribute("symbol-data"));
      });
    });
  }
  ...
  document.querySelector("#spin-select").addEventListener("change", (e) => {
    const selected = e.target.value === "true";

    if (selected) {
      document.querySelector("#display-symbol").classList.add("msc-spin");
    } else {
      document.querySelector("#display-symbol").classList.remove("msc-spin");
    }
  ...
  function updateClassDisplay() {
    const val = document
      .querySelector("#display-symbol")
      .className.replace(" !text-6xl", "");
    document.querySelector("#class-display").innerHTML = `class="${val}"`;
  }
})();
